...
class Monthly_controller extends CI_Controller {
   ...
   public function index()                      /** Note: ayaw ilisi ang sequence sa page load sa page **/
   {
        ...
        $data['title'] = 'Monthly Monitoring Records';                   
        $this->load->view('template/dashboard_header',$data);
        $this->load->view('monthly/monthly_view',$data);
    ...
    {
        $list = $this->monthly->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $monthly) {
            ...
            $row[] = $monthly->child_id;
            $row[] = $this->cis->get_child_fullname($monthly->child_id);
            
            // for shwoing what month
            if ($monthly->month == 1)
            {
                $row[] = 'January';
            }
            ...
            $row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_monthly('."'".$monthly->monthly_id."'".')"><i class="fa fa-pencil-square-o"></i> </a>
                      
                      <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Delete" onclick="delete_monthly('."'".$monthly->monthly_id."'".')"><i class="fa fa-trash"></i></a>';
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->monthly->count_all(),
                        "recordsFiltered" => $this->monthly->count_filtered(),
                        "data" => $data,
                );
